...
public class GameController {
	...
	public String fortify(
		...
		if (log.isDebugEnabled()) {
			log.debug(String.format("inside fortify, passed parameters [%s] [%s] [%s]", fromCountry, toCountry,
					numberOfArmies));
		}
		...
		if ("-none".equalsIgnoreCase(num1)) {
			if (RunningGame.getInstance().getCurrentPlayer().getPlayerModel().getCards().getList().size() >= 5) {
				RunningGame.getInstance().getCurrentPlayer().getPlayerModel()
						.setPlayingPhase("Reinforcement - Exchange Cards");
				RunningGame.getInstance().setCardExchangeCompleted(false);
				RunningGame.getInstance().getSubject().markAndNotify();
				throw new RiskGameRuntimeException("You have "
						+ RunningGame.getInstance().getCurrentPlayer().getPlayerModel().getCards().getList().size()
	...
	public String attack(
		...
		if (log.isDebugEnabled()) {
			log.debug(String.format("inside attack, passed parameters [%s] [%s] [%s]", countryNameFrom, countyNameTo,
					numDice));
		}
		...
		if ("-noattack".equalsIgnoreCase(countryNameFrom)) {
			...
			throw new RiskGameRuntimeException(
					"You need to pass either the number of dice or run attack in -allout mode");
		} else {
			try {
	...
	public String tournament(@ShellOption(value = { "-M" }, defaultValue = NONE_DEFAULT_VALUE) String mapFiles,
		...
		if (log.isDebugEnabled()) {
			log.debug(String.format("inside tournament, passed parameters [%s] [%s] [%s] [%s]", mapFiles,
					playerStrategies, noOfGames, maxTurns));
		}
