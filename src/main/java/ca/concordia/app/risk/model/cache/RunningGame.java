...
public class RunningGame extends GameModel {
  ...
  public void exchangeCardsInitialization() {
    ...
    if (cards.size() >= 3) {
      ...
      for (int i = 0; i < cards.size(); i++) {
        if (cards.get(i).equalsIgnoreCase("Infantry")) {
          infantryCount += 1;
        } else if (cards.get(i).equalsIgnoreCase("Cavalry")) {
          cavalryCount += 1;
        } else if (cards.get(i).equalsIgnoreCase("Artillery")) {
          artilleryCount += 1;
        }
      }
      if ((infantryCount >= 1 && cavalryCount >= 1 && artilleryCount >= 1)) {
        ...
        if (!"Human".equalsIgnoreCase(RunningGame.getInstance().getCurrentPlayer().getPlayerModel().getStrategy())) {
          ...
          for (int i = 0; i < cards.size(); i++) {
            if (cards.get(i).equalsIgnoreCase("Infantry") && !infantryDone) {
              cardsArray[count] = Integer.toString(i + 1);
              infantryDone = true;
              count++;
            } else if (cards.get(i).equalsIgnoreCase("Cavalry") && !cavalryDone) {
              cardsArray[count] = Integer.toString(i + 1);
              cavalryDone = true;
              count++;
            } else if (cards.get(i).equalsIgnoreCase("Artillery") && !artilleryDone) {
              cardsArray[count] = Integer.toString(i + 1);
              artilleryDone = true;
              count++;
            }
        ...
        if (!"Human".equalsIgnoreCase(RunningGame.getInstance().getCurrentPlayer().getPlayerModel().getStrategy())) {
          ...
          for (int i = 0; i < cards.size(); i++) {
            if (cards.get(i).equalsIgnoreCase(checkType)) {
              cardsArray[count] = Integer.toString(i + 1);
              count++;
            }
            if (count == 3) {
   ...
   */
  public String checkGameCompleted() {
    if (GameService.turnCount > GameService.maxTurnsCount) {
      return "Draw";
    }
    PlayerModel activePlayerModel = this.getCurrentPlayer().getPlayerModel();
    PlayerDaoImpl playerDaoImpl = new PlayerDaoImpl();
    ...
    if (totalCountryNumbers == playerCountryOwnsCount) {
      return activePlayerModel.getName();
    }
